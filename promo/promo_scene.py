"""eHardware — production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eHardware", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Hardware Designs and Products", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "KiCad, Verilog, Python".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "PCB Design Library", "Production-ready schematics and layouts for common embedded platforms", DUR["f1"]),
            ("02", "FPGA Reference Designs", "Synthesizable HDL for Xilinx and Lattice FPGAs with testbenches", DUR["f2"]),
            ("03", "Manufacturing Automation", "Automated BOM generation, DRC checks, and Gerber export pipelines", DUR["f3"]),
        ]
        ...
        components = ["Schematic", "Layout", "DRC", "BOM Gen", "Gerber Export"]
        boxes = VGroup()
        ...
        # ═══ CTA ═══
        cta_name = Text("eHardware", font_size=72, color=WHITE, weight=BOLD)
